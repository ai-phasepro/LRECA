...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train RCNN-ECA on human data with best-validation 10-fold CV"
    )
...
def main():
    ...
    prediction_path = output_dir / "rcnn_ECA_human_roc.csv"
    ...
    for fold_index in range(args.max_folds):
        ...
        for epoch_index in range(args.epochs):
            ...
            test_metrics, _, _ = run_loader(model, test_loader, loss_fn, device)
            print_metrics("Train", train_metrics)
            print_metrics("val", val_metrics)
            # Display only: test metrics never participate in checkpoint selection.
            print_metrics("Test", test_metrics)

            # Select with validation data only.  On a tie, keep the later
            # epoch, matching the corrected reproduction used for benchmarking.
            if (
                epoch >= min_epoch
                and val_metrics["acc"] >= best_val_accuracy
            ):
                best_val_accuracy = val_metrics["acc"]
        ...
        if best_state is None:
            raise RuntimeError(f"No checkpoint selected for fold {fold_index + 1}")

        model.load_state_dict(best_state)
        _, test_y_true, test_y_score = run_loader(
            model, test_loader, loss_fn, device
        )
        pd.DataFrame({"y_true": test_y_true, "y_score": test_y_score}).to_csv(
            prediction_path,
            mode="a",
            header=False,
            index=False,
