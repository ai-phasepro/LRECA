...
if __name__== '__main__':
    ...
    parser.add_argument('--save_path', type=str, default = './Save_model_personal/')
    parser.add_argument('--pos_protein_dir', type=str, default = '../Data/pos_dataset/pos_word_list_LLPS.txt')
    parser.add_argument('--neg_protein_dir', type=str, default = '../Data/neg_dataset/neg_word_list.txt')
    parser.add_argument('--list_length', type=int, default = 253, help = "for data balancing")
    parser.add_argument('--learning_rate', type=float, default = 1e-4)
    parser.add_argument('--epoch', type=int, default = 100)
    parser.add_argument('--auc_save_path', type=str, default = './Save_model_personal/auc')
    ...
    # pos_seed_list = random.sample(range(1, 1000), 10)
    # neg_seed_list = random.sample(range(1, 1000), 10)
    # f = open("../classification_output/dataset_RCNN_ECA_output/mydata_all_1507_output/RCNN_ECA_em1024_128_32_output/seed.txt", "w")
    # f.write("pos_seed_list:")
    # f.writelines(str(pos_seed_list))
    # f.write("neg_seed_list:")
    # f.writelines(str(neg_seed_list))
    # f.close()
    pos_seed_list = [20]
    neg_seed_list = [21]
    ...
    for i in range(len(pos_seed_list)):
        ...
        if not os.path.exists(args.auc_save_path):
            os.makedirs(args.auc_save_path)
        auc_save_csv = os.path.join(args.auc_save_path, './rcnn_ECA_personal_roc_{}.csv'.format((i+1)))
        ...
        while start + interval <= 1:
            ...
            for i in range(epoch):
                ...
                with torch.no_grad():
                    ...
                    test_score.to_csv(auc_save_csv, mode='a', header=False, index=False, float_format='%.4f')
