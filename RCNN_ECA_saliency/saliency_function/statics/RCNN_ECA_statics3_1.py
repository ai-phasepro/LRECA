...
if __name__== '__main__':
    ...
    test_protein_dir_path = dir_path + 'FUS_family/'
    
    # test_protein_file_name = test_protein_dir_path + 'FUS_test_statics.csv'
    test_protein_file_name = test_protein_dir_path + 'FUS_train_statics.csv'
    
    # save_matrix_path = test_protein_dir_path + 'test_matrix/'
    save_matrix_path = test_protein_dir_path + 'train_matrix/'
    ...
    # proteins_names = proteins_scores_all[::5][:-2]  # 去除test最后两个蛋白质TDP-43和LAF-1
    # proteins = proteins_scores_all[1::5][:-2]
    # scores = proteins_scores_all[2::5][:-2]


    proteins_names = proteins_scores_all[::5][:-1]  # 去除train最后一个蛋白质TDP-43
    ...
    for idx, protein_name in enumerate(proteins_names):
        ...
        if 'w' not in acid_maxscore_dict.keys():   # 有些蛋白质缺少'w'氨基酸
            acid_maxscore_dict['w'] = 0
        if 'w' not in acid_averagescore_dict.keys():
            acid_averagescore_dict['w'] = 0
    ...
    # save_test_dict(save_matrix_path+ 'maxscore_ttest_', two_acid_maxscore_ttest_dict, ttest_maxscore_sorted_pd)
    # save_test_dict(save_matrix_path+ 'avgscore_ttest_', two_acid_avgscore_ttest_dict, ttest_avgscore_sorted_pd)

    # save_test_dict(save_matrix_path+ 'maxscore_sumtest_', two_acid_maxscore_ranksumtest_dict, ranksumtest_maxscore_sorted_pd)
    # save_test_dict(save_matrix_path+ 'avgscore_sumtest_', two_acid_avgscore_ranksumtest_dict, ranksumtest_avgscore_sorted_pd)

    # drawAndSave_heatmap(save_matrix_path+ 'two_acid_ttest_', ttest_maxscore_sorted_pd, ttest_avgscore_sorted_pd)
    # drawAndSave_heatmap(save_matrix_path+ 'two_acid_sumtest_', ranksumtest_maxscore_sorted_pd, ranksumtest_avgscore_sorted_pd)
    
    # heatmap_path = test_protein_dir_path + '_'.join(dir_path.split('/')[4].split('_')[:2]) + '_test_'
    heatmap_path = test_protein_dir_path+ '_'.join(dir_path.split('/')[4].split('_')[:2]) + '_train_'
    
    drawAndSave_allheatmap(heatmap_path, ttest_avgscore_sorted_pd, ttest_maxscore_sorted_pd,
                            ranksumtest_avgscore_sorted_pd, ranksumtest_maxscore_sorted_pd)
    print('success')
