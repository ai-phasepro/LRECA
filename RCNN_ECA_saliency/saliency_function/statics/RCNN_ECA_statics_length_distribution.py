...
def compute_matrix(true_label, pre_label):
    if true_label == 1 and pre_label == 1:
        ...
        return 0, 1
    elif  true_label == 0 and pre_label == 0:
        return 1, 1


def length_distribution_statics(proteins, lengths, true_labels, predicted_labels,  length_statics_dictionary):
    for idx, protein in enumerate(proteins):
        # assert lengths[idx] == len(protein)
        length = lengths[idx]
        true_label = true_labels[idx]
        pre_label = predicted_labels[idx]
        if length >= 0 and length < 100:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['0-100'][x][y] += 1
        elif length >= 100 and length < 200:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['100-200'][x][y] += 1
        elif length >= 200 and length < 300:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['200-300'][x][y] += 1
        elif length >= 300 and length < 400:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['300-400'][x][y] += 1
        elif length >= 400 and length < 500:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['400-500'][x][y] += 1
        elif length >= 500 and length < 1000:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['500-1000'][x][y] += 1
        elif length >= 1000 and length < 1500:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['1000-1500'][x][y] += 1
        elif length >= 1500 and length < 2000:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['1500-2000'][x][y] += 1
        elif length >= 2000 and length < 2500:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['2000-2500'][x][y] += 1
        else:
            x, y = compute_matrix(true_label, pre_label)
            length_statics_dictionary['2500-infinity'][x][y] += 1
...
if __name__== '__main__':
    ...
    pos_protein_dir = '../../../processed_dataset/pos_word_list_mydata_all_1507.txt'
    # neg_protein_dir = 'neg_word_list.txt'
    neg_protein_dir = '../../../processed_dataset/neg_word_list_1479.txt'
    save_dir = '../../save_model'
    ...
    # save_csv = './mydata_all_1507_output/RCNN_ECA_em512_128_32_output/lstm_acc_rcnn_ECA_mydata_{}.csv'.format((i+1))
    # save_csv = './mydata_all_1507_output/RCNN_ECA_em1024_128_32_output/lstm_acc_rcnn_ECA_mydata_{}.csv'.format((i+1))
    ...
    # df_test.to_csv(save_csv, index=False)    # rcnn_2使用全部数据， rcnn_1使用±668数据
    ...
    length_statics_dictionary = {}
    length_statics_dictionary['0-100'] = [[0 for i in range(2)] for j in range(2)]
    length_statics_dictionary['100-200'] = [[0 for i in range(2)] for j in range(2)]
    length_statics_dictionary['200-300'] = [[0 for i in range(2)] for j in range(2)]
    length_statics_dictionary['300-400'] = [[0 for i in range(2)] for j in range(2)]
    length_statics_dictionary['400-500'] = [[0 for i in range(2)] for j in range(2)]
    length_statics_dictionary['500-1000'] = [[0 for i in range(2)] for j in range(2)]
    length_statics_dictionary['1000-1500'] = [[0 for i in range(2)] for j in range(2)]
    length_statics_dictionary['1500-2000'] = [[0 for i in range(2)] for j in range(2)]
    length_statics_dictionary['2000-2500'] = [[0 for i in range(2)] for j in range(2)]
    length_statics_dictionary['2500-infinity'] = [[0 for i in range(2)] for j in range(2)]
    ...
    while start + interval <= 1:
        ...
        rcnn = RCNN(len(vocab)+1, 1024, 100, 1, True)  # 256,100,1  hidden128:256,128,1效果较差
        rcnn = rcnn.to(device)
        print(rcnn)
        loss_fn = nn.CrossEntropyLoss()
        
        loss_fn = loss_fn.to(device)
        learning_rate = 1e-4  #1e-3, mydata上1e-4效果不行，其他数据集1e-4
        ...
        for i in range(epoch):
            ...
            with torch.no_grad():
                for input, label, length in test_dataloader:
                    ...
                    if i == 99:
                        length_distribution_statics(input.cpu(), length.cpu(), label.cpu(), predicted.cpu(),  length_statics_dictionary)
    ...
    for key, value in length_statics_dictionary.items():
        print(key)
        print(value)
