...
def create_cam_pp(feature_maps, gradients, lengths, cam_list):
    for i in range(len(feature_maps)):
        length = lengths[i]
        feature_map = feature_maps[i][:, :length]
        gradient = gradients[i][:, :length]

        grad_2 = np.power(gradient,2)
        grad_3 = np.power(gradient,3)
        alpha = grad_2 / (2 * grad_2 + (feature_map * grad_3).sum(-1, keepdims = True))
        weight = (alpha * np.max(gradient, 0)).sum(-1, keepdims = True)
        cam = (weight * feature_map).sum(0)					
        cam_list.append(cam)
...
if __name__== '__main__':
    ...
    FUS_protein_path = './RCNN_ECA_saliency/output/FUS_family_protein.xlsx'
    ...
    state_dict = torch.load('./RCNN_ECA_saliency/save_model/mydata_1507_RCNN_ECA_089-0.9930.pt')
    ...
    for input, label, length in test_dataloader:
        ...
        y_pre_test.extend(predicted_label.cpu())
