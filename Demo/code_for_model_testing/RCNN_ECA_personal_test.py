...
def readverifydata(verify_protein_path, label_value):
    ...
    label = np.full(shape=sequence.shape, fill_value=label_value)
...
if __name__== '__main__':
    ...
    if pos_test_dir:
        pos_test_name, pos_test_seq, pos_test_label = readverifydata(pos_test_dir, 1)
        test_names.extend(pos_test_name)
        test_seqs.extend(pos_test_seq)
        test_labels.extend(pos_test_label)
    if neg_test_dir:
        neg_test_name, neg_test_seq, neg_test_label = readverifydata(neg_test_dir, 0)
        test_names.extend(neg_test_name)
        test_seqs.extend(neg_test_seq)
        test_labels.extend(neg_test_label)
    if not test_seqs:
        parser.error("at least one of --pos_test_dir and --neg_test_dir must be provided")

    test_name = np.asarray(test_names)
    test_seq = test_seqs
    test_label = np.asarray(test_labels)
    train_seq,train_label = readdata(pos_protein_dir, neg_protein_dir, pos_seed, neg_seed)

    os.makedirs("../classification_output/personal_output", exist_ok=True)
    ...
    with torch.no_grad():
        ...
        test_correct = metrics.accuracy_score(y_true_data, y_pre_data)
        positive_mask = np.array(y_true_data) == 1
        negative_mask = np.array(y_true_data) == 0
        sensitivity = metrics.recall_score(y_true_data, y_pre_data, pos_label=1) if positive_mask.any() else np.nan
        specificity = metrics.recall_score(y_true_data, y_pre_data, pos_label=0) if negative_mask.any() else np.nan
        auc = metrics.roc_auc_score(y_true_data, y_score_data) if positive_mask.any() and negative_mask.any() else np.nan
        result = pd.DataFrame([{
            'acc': test_correct,
            'sen': sensitivity,
            'spe': specificity,
        ...
        result.to_csv(result_save_csv, mode='a', header=False, index=False, float_format='%.4f')
        print(result)
